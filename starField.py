import random, math, pygame, sys, variables


#MAKE DIFFERERNT COLORS
class Stars(object):
    def __init__(self, x, y, screen, mode):
        self.screen = screen
        self.x = x
        self.y = y
    ...
    def draw(self):
        ...
        if self.mode == "Color":
            ...
            color = (255, 255, 255)
        pygame.draw.circle(self.screen, color, (cx, cy), r, 0)
...
class StarField(object):
    ...
    def onKeyPressed(self, key):
        ...
        if key[pygame.K_s]: #slows down
            self.slowDown()
        if key[pygame.K_q]:  #quits
            variables.playSF = False
        if key[pygame.K_c]:  #changes color mode
            if self.mode == "Color":
                self.mode = "White"
            elif self.mode == "White":
                self.mode = "Color"
    ...
    def onTimerFired(self):
        #creates 7 stars every timerfired
        for i in xrange(7):
            newStar=Stars(self.x, self.y, self.screen, self.mode)
